...
funcionE.empleadosAccessAll = (acc_andon, sign, callback) => {
    ...
    dbE.query(`SELECT acc_id FROM del_accesos WHERE acc_mandriles ${sign}${acc_andon}`, function (err, result, fields) {
        if (err) {
            callback(err, null);
        } else {
...
funcionE.empleadosAccesos = (callback) => {
    ...
    dbE.query(`SELECT acc_id, acc_mandriles FROM del_accesos WHERE acc_mandriles>0`, function (err, result, fields) {
        if (err) {
            callback(err, null);
        } else {
...
funcionE.empleadosDeleteAcceso = (acc_id, callback) => {
    ...
    dbE.query(`UPDATE del_accesos SET acc_mandriles=0 WHERE acc_id=${acc_id}`, function (err, result, fields) {
            if (err) {
...
funcionE.empleadosInsertAcceso = (acc_id, acc_mandriles, callback) => {

    dbE.query(`INSERT INTO del_accesos (acc_id, acc_mandriles)
    VALUES('${acc_id}',${acc_mandriles})
    ON DUPLICATE KEY UPDATE acc_mandriles= VALUES(acc_mandriles)`, function (err, result, fields) {
            if (err) {
